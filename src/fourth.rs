...
impl<T> List<T> {
    ...
    pub fn push_front(&mut self, elem: T) {
        ...
        match self.head.take() {
            ...
            None => {
                self.tail = Some(new_head.clone()); // clone the Rc so both tail and head point to the same thing
                self.head = Some(new_head);
            }
        }
    }
    ...
    pub fn pop_front(&mut self) -> Option<T> {
        self.head.take().map(|old_head| {
            ...
            // Ah dang. into_inner wants to move out the RefCell, but we can't, because it's in an Rc.
